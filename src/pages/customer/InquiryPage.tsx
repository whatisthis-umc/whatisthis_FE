...
const InquiryPage = () => {
  ...
  const fetchDetailIfNeeded = async (inquiryId: number) => {
    ...
    try {
      ...
      // 권한 없는 경우(작성자 아님) - 403, 500, 401 에러인 경우 처리
      // 401: 토큰 문제, 403: 일반적인 권한 없음, 500: 서버 내부 오류(배포환경에서 권한 없음으로 처리)
      if (e?.response?.status === 401 || e?.response?.status === 403 || e?.response?.status === 500) {
        // 비밀글 권한 부족 모달 표시
