...
const CommunityPostPage = () => {
  ...
  const actionBtnClass =
    "inline-flex items-center justify-center w-[160px] h-[54px] rounded-[32px] px-[16px] py-[12px] text-[16px] font-normal bg-[#0080FF] text-white cursor-pointer transition hover:opacity-90 active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#99CCFF] disabled:opacity-50 disabled:pointer-events-none";
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 py-6 sm:py-8 font-[Pretendard]">
      <div className="flex flex-col lg:flex-row gap-[40px] sm:gap-[60px] lg:gap-[80px]">
        <div className="flex flex-col items-center gap-3 sm:gap-4 w-full max-w-[681.76px]">
          <div className="w-full h-[200px] sm:h-[260px] lg:h-[681.76px] bg-[#E6E6E6] rounded-[20px] sm:rounded-[24px] lg:rounded-[32px] flex justify-center items-center relative overflow-hidden">
            {image ? (
              <>
                <img src={URL.createObjectURL(image)} alt="preview" className="w-full h-full object-cover" />
                <img src={cancelIcon} alt="cancel" onClick={() => setImage(null)} className="absolute top-2 right-2 w-5 h-5 sm:w-6 sm:h-6 cursor-pointer opacity-80 hover:opacity-100" />
              </>
            ) : (
              <img src={addPhotoIcon} alt="upload" className="w-[40px] h-[40px] sm:w-[60px] sm:h-[60px] opacity-50" />
            )}
          </div>

          <label htmlFor="fileInput" className="w-full h-[48px] sm:h-[54px] bg-[#0080FF] text-white rounded-[20px] sm:rounded-[24px] lg:rounded-[32px] px-[24px] sm:px-[32px] flex items-center justify-center gap-2 cursor-pointer text-[15px] sm:text-[16px] font-normal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#99CCFF] hover:opacity-90">
            <img src={addPhotoIcon} alt="add" className="w-4 h-4 sm:w-5 sm:h-5" />
            파일에서 업로드
          </label>
          <input id="fileInput" type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
        </div>

        <div className="flex flex-col gap-4 sm:gap-6 w-full max-w-[681.76px]">
          <div className="relative">
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="w-full h-[56px] sm:h-[64px] lg:h-[72px] rounded-[20px] sm:rounded-[24px] lg:rounded-[32px] border border-[#E6E6E6] px-[20px] sm:px-[24px] pr-[40px] sm:pr-[48px] text-[#333] text-[15px] sm:text-[16px] bg-[#E6E6E6] font-bold appearance-none cursor-pointer"
            >
              <option value="" className="font-bold text-[15px] sm:text-[16px]">유형</option>
              {categoryOptions.map((opt) => (
                <option key={opt} value={opt} className="font-bold text-[15px] sm:text-[16px]">
                  {opt}
                </option>
              ))}
            </select>
            <img src={bArrowDown} alt="arrow" className="pointer-events-none absolute right-[20px] sm:right-[24px] top-1/2 -translate-y-1/2 w-[10px] h-[6px] sm:w-[12px] sm:h-[8px]" />
          </div>
          ...
          <input
            ...
            maxLength={20}
            className="text-[20px] sm:text-[22px] lg:text-[24px] font-bold text-[#1d1d1d] placeholder:text-[#3e3e3e] border border-[#E6E6E6] rounded-[20px] sm:rounded-[24px] lg:rounded-[32px] px-[20px] sm:px-[24px] py-[10px] sm:py-[12px]"
          />
          ...
          <textarea
            ...
            className="border border-[#E6E6E6] rounded-[20px] sm:rounded-[24px] lg:rounded-[32px] px-[20px] sm:px-[24px] py-[20px] sm:py-[24px] text-[15px] sm:text-[16px] font-normal text-[#333] min-h-[160px] sm:min-h-[180px] lg:min-h-[200px]"
          />
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {tags.length < 5 && (
                <>
                  ...
                  />
                  <button type="button" onClick={handleAddTag} className="text-[#0080FF] text-sm font-normal px-3 py-1 border border-[#0080FF] rounded-full cursor-pointer hover:opacity-90">+</button>
                </>
              )}
            </div>
          </div>
